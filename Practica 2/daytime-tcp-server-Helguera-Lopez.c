...
int main(int argc, char *argv[]){
    ...
    tcpSocket = socket(AF_INET, SOCK_STREAM, 0);         //Se inicia el socket TCP, que esta siempre a la escucha
    ...
    while(1){           //el proceso padre esperando conexiones
        ...
        pid=fork();                             //Se crea el proceso hijo porque ha llegado una nueva conexion
        if(pid==0){      
...
void signal_handler(int sig){
    signal(sig, SIG_IGN);
    shutdown(tcpSocket, SHUT_RDWR);                             //Se comunica al cliente que se va a cerrar el servidor
    shutdown(newtcpSocket, SHUT_RDWR);
    recv(newtcpSocket,comprobacion,1024,0);                     
    recv(tcpSocket,comprobacion,1024,0);
    close(tcpSocket);                                           //Se cierran definitivamente los sockets
    close(newtcpSocket);
